...
def get_region_info(reg_code):
    response = requests.get(API_ENDPOINT + f"ref/region/info/{reg_code}", headers=headers, params={"regionNameFormat":"detailed"})
    # Check the response status and print the data
    if response.status_code == 200:
        data = response.json()  # Parse the JSON response
        return data
...
def get_hotspot_info(locId):
    response = requests.get(API_ENDPOINT + f"ref/hotspot/info/{locId}", headers=headers)
    # Check the response status and print the data
    if response.status_code == 200:
        data = response.json()  # Parse the JSON response
        return data
